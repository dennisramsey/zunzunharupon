import { getPath, getNodeEntry, getNodeDescendants, isVoid, match, getAboveNode, isAncestor, getEdgePoints, isText, getLastNode, getMarks, getNextNode, getStartPoint, getPointBefore, getPoint, getEditorString, getVoidNode, getPointAfter, findNode, getPreviousNode, isBlock, getNodeEntries, queryNode, getEndPoint, getNode, getNodeString, isInline, getParentNode, isEndPoint, getRange, isStartPoint, isExpanded, someNode, insertNodes, getQueryOptions, moveNodes, unsetNodes, withoutNormalizing, getNodeChildren, removeNodes, removeEditorMark, select, unwrapNodes, wrapNodes } from '@udecode/slate';
import { Span, Range, Path, Point } from 'slate';
import { isDefined, escapeRegExp } from '@udecode/utils';
...
const getPointNextToVoid = (editor, {
  ...
  if (startVoid) {
    ...
    if (blockAbove) {
      const nextPoint = after ? getPointAfter(editor, at) : getPointBefore(editor, at);
...
const queryEditor = (editor, {
  ...
  if (allows.length > 0 && !someNode(editor, {
    at,
    match: {
      type: allows
    }
  ...
  if (excludes.length > 0 && someNode(editor, {
    at,
    match: {
      type: excludes
    }
