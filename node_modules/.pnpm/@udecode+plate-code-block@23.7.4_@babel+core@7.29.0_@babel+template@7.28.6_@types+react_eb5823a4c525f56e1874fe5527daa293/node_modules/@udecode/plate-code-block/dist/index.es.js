import { getPlugin, getParentNode, getNodeString, getPluginType, someNode, getAboveNode, isElement, getStartPoint, isExpanded, getRange, getEditorString, insertText, getPointAfter, deleteText, Hotkeys, getNodeEntries, withoutNormalizing, select, findNode, getChildren, setNodes, isStartPoint, isSelectionAtBlockStart, setElements, wrapNodes, insertElements, isBlockAboveEmpty, ELEMENT_DEFAULT, unwrapNodes, createPluginFactory, KEY_DESERIALIZE_HTML, usePlateEditorRef, useElement, getPluginOptions, findNodePath } from '@udecode/plate-common';
...
const useCodeBlockComboboxState = () => {
  var _element$lang;
  const editor = usePlateEditorRef();
  const readOnly = useReadOnly();
  const element = useElement();
  const [value, setValue] = React.useState((_element$lang = element.lang) !== null && _element$lang !== void 0 ? _element$lang : 'text');
  const {
    syntaxPopularFirst
  } = getPluginOptions(editor, ELEMENT_CODE_BLOCK);
  useEffect(() => {
    var _element$lang2;
    setValue((_element$lang2 = element.lang) !== null && _element$lang2 !== void 0 ? _element$lang2 : 'text');
  }, [element.lang]);
  return {
    readOnly,
    syntaxPopularFirst,
...
const useCodeBlockElementState = ({
  element
}) => {
  const editor = usePlateEditorRef();
...
export { CODE_BLOCK_LANGUAGES, CODE_BLOCK_LANGUAGES_POPULAR, ELEMENT_CODE_BLOCK, ELEMENT_CODE_LINE, ELEMENT_CODE_SYNTAX, createCodeBlockPlugin, decorateCodeLine, deleteStartSpace, deserializeHtmlCodeBlock, getCodeLineEntry, getCodeLineType, getIndentDepth, indentCodeLine, insertCodeBlock, insertCodeLine, insertEmptyCodeBlock, insertFragmentCodeBlock, isCodeBlockEmpty, isSelectionAtCodeBlockStart, normalizeCodeBlock, onKeyDownCodeBlock, outdentCodeLine, toggleCodeBlock, unwrapCodeBlock, useCodeBlockCombobox, useCodeBlockComboboxState, useCodeBlockElementState, useCodeSyntaxLeaf, withCodeBlock };
