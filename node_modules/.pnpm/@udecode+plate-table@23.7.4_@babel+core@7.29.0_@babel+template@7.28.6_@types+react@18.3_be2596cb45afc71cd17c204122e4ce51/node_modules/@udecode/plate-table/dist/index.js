...
const getCellInNextTableRow = (editor, currentRowPath) => {
  ...
  const nextRow = plateCommon.getNodeEntry(editor, slate.Path.next(currentRowPath));
  if (!nextRow) return;
  ...
  if (nextCell && nextCellPath) {
    return plateCommon.getNodeEntry(editor, nextCellPath);
  }
};

const getCellInPreviousTableRow = (editor, currentRowPath) => {
  var _previousRowNode$chil;
  const prevPath = plateCommon.getPreviousPath(currentRowPath);
  if (!prevPath) return;
  const previousRow = plateCommon.getNodeEntry(editor, prevPath);
  ...
  if (previousCell && previousCellPath) {
    return plateCommon.getNodeEntry(editor, previousCellPath);
  }
};
 ...
 * Get td and th types
 */
const getCellTypes = editor => plateCommon.getPluginTypes(editor, [ELEMENT_TD, ELEMENT_TH]);
...
const getEmptyCellNode = (editor, {
  ...
  return {
    type: header ? plateCommon.getPluginType(editor, ELEMENT_TH) : plateCommon.getPluginType(editor, ELEMENT_TD),
...
const getEmptyRowNode = (editor, {
  ...
  return {
    type: plateCommon.getPluginType(editor, ELEMENT_TR),
    children: Array.from({
      length: colCount
    }).fill(colCount).map(() => getEmptyCellNode(editor, options))
  };
};
...
const getEmptyTableNode = (editor, {
  ...
  const rows = Array.from({
    length: rowCount
  }).fill(rowCount).map(() => getEmptyRowNode(editor, {
    header,
    colCount,
    newCellChildren
  }));
  return {
    type: plateCommon.getPluginType(editor, ELEMENT_TABLE),
    children: rows
  };
};
...
const getLeftTableCell = (editor, {
  ...
  if (!cellPath) {
    var _findNode;
    cellPath = (_findNode = plateCommon.findNode(editor, {
      match: {
  ...
  if (!cellIndex) return;
  const prevCellPath = slate.Path.previous(cellPath);
  return plateCommon.getNodeEntry(editor, prevCellPath);
};

const getNextTableCell = (editor, currentCell, currentPath, currentRow) => {
  const cell = plateCommon.getNodeEntry(editor, slate.Path.next(currentPath));
  if (cell) return cell;
...
const getPreviousTableCell = (editor, currentCell, currentPath, currentRow) => {
  const prevPath = plateCommon.getPreviousPath(currentPath);
  if (!prevPath) {
    const [, currentRowPath] = currentRow;
    return getCellInPreviousTableRow(editor, currentRowPath);
  }
  const cell = plateCommon.getNodeEntry(editor, prevPath);
  if (cell) return cell;
};

const getTableAbove = (editor, options) => plateCommon.getBlockAbove(editor, {
  match: {
    type: plateCommon.getPluginType(editor, ELEMENT_TABLE)
  },
  ...options
...
const getTableColumnCount = tableNode => {
  var _$children$length, _tableNode$children, _tableNode$children$, _tableNode$children$$;
  return (_$children$length = (_tableNode$children = tableNode.children) === null || _tableNode$children === void 0 ? void 0 : (_tableNode$children$ = _tableNode$children[0]) === null || _tableNode$children$ === void 0 ? void 0 : (_tableNode$children$$ = _tableNode$children$.children) === null || _tableNode$children$$ === void 0 ? void 0 : _tableNode$children$$.length) !== null && _$children$length !== void 0 ? _$children$length : 0;
};
...
const getTableColumnIndex = (editor, cellNode) => {
  var _getParentNode;
  const path = plateCommon.findNodePath(editor, cellNode);
  if (!path) return 0;
  const [trNode] = (_getParentNode = plateCommon.getParentNode(editor, path)) !== null && _getParentNode !== void 0 ? _getParentNode : [];
  if (!trNode) return 0;
  let colIndex = 0;
...
const getTableEntries = (editor, {
  ...
  if (!at) return;
  const cellEntry = plateCommon.findNode(editor, {
    at,
    match: {
      type: getCellTypes(editor)
    }
  ...
  const rowEntry = plateCommon.getAboveNode(editor, {
    at: cellPath,
    match: {
      type: plateCommon.getPluginType(editor, ELEMENT_TR)
    }
  ...
  const tableEntry = plateCommon.getAboveNode(editor, {
    at: rowPath,
    match: {
      type: plateCommon.getPluginType(editor, ELEMENT_TABLE)
    }
...
const getTableGridByRange = (editor, {
  ...
  while (true) {
    ...
    const cell = plateCommon.getNode(editor, cellPath);
    if (!cell) break;
...
const getTableGridAbove = (editor, {
  ...
  const edges = plateCommon.getEdgeBlocksAbove(editor, {
    match: {
      type: getCellTypes(editor)
    },
    ...options
  ...
  if (edges) {
    ...
    if (!slate.Path.equals(start[1], end[1])) {
...
const getTableOverriddenColSizes = (tableNode, colSizeOverrides) => {
  ...
  const colSizes = (tableNode.colSizes ? [...tableNode.colSizes] : Array.from({
    length: colCount
  }).fill(0)).map((size, index) => {
...
const getTableRowIndex = (editor, cellNode) => {
  const path = plateCommon.findNodePath(editor, cellNode);
  if (!path) return 0;
  const rowPath = slate.Path.parent(path);
  return rowPath.at(-1);
};
...
const getTopTableCell = (editor, {
  ...
  const cellAbovePath = [...slate.Path.parent(slate.Path.parent(cellPath)), rowIndex - 1, cellIndex];
  return plateCommon.getNodeEntry(editor, cellAbovePath);
};
...
const isTableBorderHidden = (editor, border) => {
  ...
  return ((_findNode = plateCommon.findNode(editor, {
    match: {
      type: getCellTypes(editor)
    }
...
const deleteColumn = editor => {
  if (plateCommon.someNode(editor, {
    ...
    const tdEntry = plateCommon.getAboveNode(editor, {
      match: {
        type: [plateCommon.getPluginType(editor, ELEMENT_TD), plateCommon.getPluginType(editor, ELEMENT_TH)]
      }
    });
    const trEntry = plateCommon.getAboveNode(editor, {
      match: {
        type: plateCommon.getPluginType(editor, ELEMENT_TR)
      }
    });
...
const deleteRow = editor => {
  if (plateCommon.someNode(editor, {
    ...
    const currentTableItem = plateCommon.getAboveNode(editor, {
      match: {
        type: plateCommon.getPluginType(editor, ELEMENT_TABLE)
      }
    });
    const currentRowItem = plateCommon.getAboveNode(editor, {
      match: {
        type: plateCommon.getPluginType(editor, ELEMENT_TR)
      }
    });
...
const deleteTable = editor => {
  if (plateCommon.someNode(editor, {
    ...
    const tableItem = plateCommon.getAboveNode(editor, {
      match: {
        type: plateCommon.getPluginType(editor, ELEMENT_TABLE)
      }
    });
...
const insertTable = (editor, {
  ...
  plateCommon.withoutNormalizing(editor, () => {
    if (!plateCommon.someNode(editor, {
      ...
      if (editor.selection) {
        const tableEntry = plateCommon.getBlockAbove(editor, {
          match: {
            type: plateCommon.getPluginType(editor, ELEMENT_TABLE)
          }
        });
        if (!tableEntry) return;
        plateCommon.selectEditor(editor, {
          at: plateCommon.getStartPoint(editor, tableEntry[1])
        });
      }
    }
  });
...
const insertTableColumn = (editor, {
  ...
  const cellEntry = fromCell ? plateCommon.findNode(editor, {
    at: fromCell,
    match: {
      type: getCellTypes(editor)
    }
  }) : plateCommon.getBlockAbove(editor, {
    match: {
      type: getCellTypes(editor)
    }
  });
  ...
  if (slate.Path.isPath(at)) {
    ...
    nextCellPath = slate.Path.next(cellPath);
  ...
  const {
    ...
    initialTableWidth,
    minColumnWidth
  } = plateCommon.getPluginOptions(editor, ELEMENT_TABLE);
  plateCommon.withoutNormalizing(editor, () => {
    ...
    tableNode.children.forEach((row, rowIndex) => {
      ...
      const isHeaderRow = header === undefined ? row.children.every(c => c.type === plateCommon.getPluginType(editor, ELEMENT_TH)) : header;
      plateCommon.insertElements(editor, getEmptyCellNode(editor, {
        header: isHeaderRow,
        newCellChildren
      }), {
        at: insertCellPath,
...
const insertTableRow = (editor, {
  ...
  const trEntry = fromRow ? plateCommon.findNode(editor, {
    at: fromRow,
    match: {
      type: plateCommon.getPluginType(editor, ELEMENT_TR)
    }
  ...
  const getEmptyRowNode = () => ({
    type: plateCommon.getPluginType(editor, ELEMENT_TR),
    children: trNode.children.map((_, i) => getEmptyCellNode(editor, {
      header: header !== null && header !== void 0 ? header : tableEntry[0].children.every(n => n.children[i].type === ELEMENT_TH),
      ...newCellChildren
    }))
  });
  plateCommon.withoutNormalizing(editor, () => {
    plateCommon.insertElements(editor, getEmptyRowNode(), {
      at: slate.Path.isPath(at) ? at : slate.Path.next(trPath)
    });
  });
  if (!disableSelect) {
    const cellEntry = plateCommon.getBlockAbove(editor, {
      match: {
        type: getCellTypes(editor)
      }
    });
    ...
    plateCommon.select(editor, nextCellPath);
  }
};
...
const moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if (plateCommon.hasNode(editor, anchorPath) && plateCommon.hasNode(editor, focusPath)) {
        plateCommon.select(editor, {
          anchor: plateCommon.getStartPoint(editor, anchorPath),
          focus: plateCommon.getStartPoint(editor, focusPath)
        });
      }
      return true;
    }
  ...
  if (cellEntry) {
    ...
    if (plateCommon.hasNode(editor, nextCellPath)) {
      plateCommon.select(editor, plateCommon.getStartPoint(editor, nextCellPath));
    } else {
      const tablePath = cellPath.slice(0, -2);
      if (reverse) {
        plateCommon.withoutNormalizing(editor, () => {
          plateCommon.select(editor, plateCommon.getStartPoint(editor, tablePath));
        ...
        plateCommon.withoutNormalizing(editor, () => {
          plateCommon.select(editor, plateCommon.getEndPoint(editor, tablePath));
          plateCommon.moveSelection(editor);
        });
      }
    }
...
const overrideSelectionFromCell = (editor, newSelection) => {
  ...
  if (!editor.currentKeyboardEvent || !['up', 'down', 'shift+up', 'shift+right', 'shift+down', 'shift+left'].some(key => {
    ...
    if (valid) hotkey = key;
    return valid;
  }) || !((_editor$selection = editor.selection) !== null && _editor$selection !== void 0 && _editor$selection.focus) || !(newSelection !== null && newSelection !== void 0 && newSelection.focus) || !plateCommon.isRangeAcrossBlocks(editor, {
    at: {
  ...
  if (edge && !plateCommon.isRangeInSameBlock(editor, {
    at: editor.selection,
    match: {
      type: getCellTypes(editor)
    }
...
const setBorderSize = (editor, size, {
  ...
  const setNodesOptions = {
    match: n => plateCommon.isElement(n) && getCellTypes(editor).includes(n.type)
  };
  if (border === 'top') {
...
const setTableColSize = (editor, {
  ...
  const table = plateCommon.findNode(editor, {
    match: {
      type: ELEMENT_TABLE
    },
    ...options
  ...
  const colSizes = tableNode.colSizes ? [...tableNode.colSizes] : Array.from({
    length: getTableColumnCount(tableNode)
  }).fill(0);
...
const onKeyDownTable = (editor, {
  ...
  if (isTab || isUntab) {
    ...
    if (isUntab) {
      ...
      if (previousCell) {
        ...
        plateCommon.select(editor, previousCellPath);
      }
    } else if (isTab) {
      // move right with tab
  ...
  if (isHotkey('mod+a', e)) {
    const res = plateCommon.getAboveNode(editor, {
      match: {
        type
      }
    });
    ...
    plateCommon.select(editor, tablePath);
    e.preventDefault();
    e.stopPropagation();
  }
};
...
const preventDeleteTableCell = (editor, {
  ...
  const getPoint = reverse ? plateCommon.getEndPoint : plateCommon.getStartPoint;
  const getNextPoint = reverse ? plateCommon.getPointAfter : plateCommon.getPointBefore;
  if (plateCommon.isCollapsed(selection)) {
    ...
    if (cellEntry) {
      ...
      const nextCellEntry = plateCommon.getBlockAbove(editor, {
        match: {
          type: getCellTypes(editor)
        },
        at: nextPoint
...
const withDeleteTable = editor => {
  ...
  editor.deleteFragment = direction => {
    plateCommon.isRangeInSameBlock(editor, {
      match: n => n.type === plateCommon.getPluginType(editor, ELEMENT_TABLE)
    });
    if (plateCommon.isRangeInSameBlock(editor, {
      match: n => n.type === plateCommon.getPluginType(editor, ELEMENT_TABLE)
    })) {
      const cellEntries = getTableGridAbove(editor, {
        format: 'cell'
      ...
      if (cellEntries.length > 1) {
        plateCommon.withoutNormalizing(editor, () => {
          ...
          plateCommon.select(editor, {
            anchor: plateCommon.getStartPoint(editor, cellEntries[0][1]),
            focus: plateCommon.getEndPoint(editor, cellEntries.at(-1)[1])
          });
        });
        return;
      }
...
const withGetFragmentTable = editor => {
  ...
  editor.getFragment = () => {
    ...
    fragment.forEach(node => {
      if (node.type === plateCommon.getPluginType(editor, ELEMENT_TABLE)) {
...
const withInsertFragmentTable = (editor, {
  ...
  const myEditor = plateCommon.getTEditor(editor);
  myEditor.insertFragment = fragment => {
    const insertedTable = fragment.find(n => n.type === plateCommon.getPluginType(editor, ELEMENT_TABLE));
    if (!insertedTable) {
      var _editor$selection;
          ...
          plateCommon.withoutNormalizing(editor, () => {
            ...
            insertedRows.forEach(row => {
              ...
              if (!initRow) {
                ...
                if (!plateCommon.hasNode(editor, cellPath)) {
            ...
            if (lastCellPath) {
              plateCommon.select(editor, {
                anchor: plateCommon.getStartPoint(editor, startCellPath),
                focus: plateCommon.getEndPoint(editor, lastCellPath)
              });
            }
          });
          return;
...
const withNormalizeTable = editor => {
  ...
  myEditor.normalizeNode = ([node, path]) => {
    if (plateCommon.isElement(node)) {
      ...
      if (node.type === plateCommon.getPluginType(editor, ELEMENT_TR)) {
        const parentEntry = plateCommon.getParentNode(editor, path);
        if ((parentEntry === null || parentEntry === void 0 ? void 0 : parentEntry[0].type) !== plateCommon.getPluginType(editor, ELEMENT_TABLE)) {
          plateCommon.unwrapNodes(editor, {
            at: path
          });
          return;
      ...
      if (getCellTypes(editor).includes(node.type)) {
        ...
        const parentEntry = plateCommon.getParentNode(editor, path);
        if ((parentEntry === null || parentEntry === void 0 ? void 0 : parentEntry[0].type) !== plateCommon.getPluginType(editor, ELEMENT_TR)) {
          plateCommon.unwrapNodes(editor, {
            at: path
          });
          return;
        }
        if (plateCommon.isText(children[0])) {
          plateCommon.wrapNodeChildren(editor, editor.blockFactory({}, path), {
            at: path
          });
          return;
        }
...
const withSelectionTable = editor => {
  ...
  editor.apply = op => {
    if (op.type === 'set_selection' && op.newProperties) {
      ...
      if (slate.Range.isRange(newSelection) && plateCommon.isRangeAcrossBlocks(editor, {
        at: newSelection,
        match: n => n.type === plateCommon.getPluginType(editor, ELEMENT_TABLE)
      })) {
        const anchorEntry = plateCommon.getBlockAbove(editor, {
          at: newSelection.anchor,
          match: n => n.type === plateCommon.getPluginType(editor, ELEMENT_TABLE)
        });
        if (anchorEntry) {
          const [, anchorPath] = anchorEntry;
          const isBackward = slate.Range.isBackward(newSelection);
          if (isBackward) {
            op.newProperties.focus = plateCommon.getStartPoint(editor, anchorPath);
          } else {
            const pointBefore = plateCommon.getPointBefore(editor, anchorPath);
            // if the table is the first block
            if (pointBefore) {
              op.newProperties.focus = plateCommon.getEndPoint(editor, anchorPath);
            }
          }
        } else {
          const focusEntry = plateCommon.getBlockAbove(editor, {
          ...
          if (focusEntry) {
            ...
            const isBackward = slate.Range.isBackward(newSelection);
            if (isBackward) {
              const startPoint = plateCommon.getStartPoint(editor, focusPath);
              const pointBefore = plateCommon.getPointBefore(editor, startPoint);
              op.newProperties.focus = pointBefore !== null && pointBefore !== void 0 ? pointBefore : startPoint;
            } else {
              op.newProperties.focus = plateCommon.getEndPoint(editor, focusPath);
            }
          }
        }
      }
...
const withSetFragmentDataTable = editor => {
  ...
  editor.setFragmentData = (data, originEvent) => {
    ...
    const selectionStart = slate.Path.compare(initialSelection.anchor.path, initialSelection.focus.path) < 1 ? initialSelection.anchor : initialSelection.focus;
    const [tableNode, tablePath] = tableEntry;
    ...
    plateCommon.withoutNormalizing(editor, () => {
      tableRows.forEach((row, rowIndex) => {
        ...
        const cellStrings = [];
        const rowElement = row.type === plateCommon.getPluginType(editor, ELEMENT_TH) ? document.createElement('th') : document.createElement('tr');
        rowCells.forEach((_, cellIndex) => {
          const cellPath = rowPath.concat(x + cellIndex);

          // select cell by cell
          plateCommon.select(editor, {
            anchor: plateCommon.getStartPoint(editor, cellPath),
            focus: plateCommon.getEndPoint(editor, cellPath)
          });
      ...
      plateCommon.select(editor, initialSelection);
 ...
 */
const createTablePlugin = plateCommon.createPluginFactory({
  key: ELEMENT_TABLE,
  isElement: true,
  handlers: {
...
const useOverrideSizeFactory = setOverrides => react.useCallback((index, size) => {
...
const useTableColSizes = (tableNode, {
  ...
  react.useEffect(() => {
    ...
    if (enableUnsetSingleColSize && colCount < 2 && (_tableNode$colSizes = tableNode.colSizes) !== null && _tableNode$colSizes !== void 0 && _tableNode$colSizes.length) {
      plateCommon.unsetNodes(editor, 'colSizes', {
        at: plateCommon.findNodePath(editor, tableNode)
      });
    }
...
const useTableCellElementResizableState = ({
  colIndex,
  rowIndex,
  step,
  stepX = step,
...
const useTableCellElementResizable = ({
  ...
  const tableElement = plateCommon.useElement(ELEMENT_TABLE);
  const {
    minColumnWidth = 0
  } = plateCommon.getPluginOptions(editor, ELEMENT_TABLE);
  ...
  const setColSize = react.useCallback((colIndex, width) => {
    setTableColSize(editor, {
      colIndex,
      width
    }, {
      at: plateCommon.findNodePath(editor, element)
    });
  ...
  /* eslint-disable @typescript-eslint/no-shadow */
  const setRowSize = react.useCallback((rowIndex, height) => {
  ...
  const setMarginLeft = react.useCallback(marginLeft => {
  ...
  const handleResizeRight = react.useCallback(({
    ...
    const currentNew = roundCellSizeToStep(plateResizable.resizeLengthClampStatic(currentInitial + delta, {
      min: minColumnWidth,
  ...
  const handleResizeBottom = react.useCallback(event => {
  ...
  const handleResizeLeft = react.useCallback(event => {
    const initial = colSizesWithoutOverrides[colIndex];
    const complement = width => initial + marginLeft - width;
    const newMargin = roundCellSizeToStep(plateResizable.resizeLengthClampStatic(marginLeft + event.delta, {
      min: 0,
...
const useIsCellSelected = element => {
  ...
  return react.useMemo(() => !!(selectedCells !== null && selectedCells !== void 0 && selectedCells.includes(element)), [element, selectedCells]);
};
...
const useTableCellElementState = ({
  ...
  var _ref, _rowSizeOverrides$get, _tableElement$childre;
  const editor = plateCommon.usePlateEditorRef();
  const cellElement = plateCommon.useElement();
  ...
  const selectedCells = useTableStore().get.selectedCells();
  const tableElement = plateCommon.useElement(ELEMENT_TABLE);
  const rowElement = plateCommon.useElement(ELEMENT_TR);
  ...
  const isFirstRow = ((_tableElement$childre = tableElement.children) === null || _tableElement$childre === void 0 ? void 0 : _tableElement$childre[0]) === rowElement;
...
const useSelectedCells = () => {
  ...
  }, [editor, editor === null || editor === void 0 ? void 0 : editor.selection, readOnly, selectedCells, setSelectedCells]);
};
...
exports.getCellTypes = getCellTypes;
...
exports.getLeftTableCell = getLeftTableCell;
exports.getNextTableCell = getNextTableCell;
exports.getOnSelectTableBorderFactory = getOnSelectTableBorderFactory;
exports.getPreviousTableCell = getPreviousTableCell;
exports.getTableAbove = getTableAbove;
exports.getTableCellBorders = getTableCellBorders;
...
exports.getTableGridAbove = getTableGridAbove;
exports.getTableGridByRange = getTableGridByRange;
exports.getTableOverriddenColSizes = getTableOverriddenColSizes;
exports.getTableRowIndex = getTableRowIndex;
exports.getTopTableCell = getTopTableCell;
exports.insertTable = insertTable;
exports.insertTableColumn = insertTableColumn;
exports.insertTableRow = insertTableRow;
exports.isTableBorderHidden = isTableBorderHidden;
exports.keyShiftEdges = keyShiftEdges;
exports.moveSelectionFromCell = moveSelectionFromCell;
exports.onKeyDownTable = onKeyDownTable;
exports.overrideSelectionFromCell = overrideSelectionFromCell;
exports.preventDeleteTableCell = preventDeleteTableCell;
exports.roundCellSizeToStep = roundCellSizeToStep;
exports.setBorderSize = setBorderSize;
exports.setTableColSize = setTableColSize;
exports.setTableMarginLeft = setTableMarginLeft;
exports.setTableRowSize = setTableRowSize;
exports.tableStore = tableStore;
exports.useIsCellSelected = useIsCellSelected;
exports.useOverrideColSize = useOverrideColSize;
exports.useOverrideMarginLeft = useOverrideMarginLeft;
exports.useOverrideRowSize = useOverrideRowSize;
exports.useSelectedCells = useSelectedCells;
exports.useTableBordersDropdownMenuContentState = useTableBordersDropdownMenuContentState;
exports.useTableCellElement = useTableCellElement;
exports.useTableCellElementResizable = useTableCellElementResizable;
exports.useTableCellElementResizableState = useTableCellElementResizableState;
exports.useTableCellElementState = useTableCellElementState;
exports.useTableColSizes = useTableColSizes;
exports.useTableElement = useTableElement;
exports.useTableElementState = useTableElementState;
exports.useTableStore = useTableStore;
exports.withDeleteTable = withDeleteTable;
exports.withGetFragmentTable = withGetFragmentTable;
exports.withInsertFragmentTable = withInsertFragmentTable;
exports.withInsertTextTable = withInsertTextTable;
exports.withNormalizeTable = withNormalizeTable;
exports.withSelectionTable = withSelectionTable;
exports.withSetFragmentDataTable = withSetFragmentDataTable;
