...
const someList = (editor, type) => {
  var _getListItemEntry, _getListItemEntry$lis;
  return ((_getListItemEntry = getListItemEntry(editor)) === null || _getListItemEntry === void 0 ? void 0 : (_getListItemEntry$lis = _getListItemEntry.list) === null || _getListItemEntry$lis === void 0 ? void 0 : _getListItemEntry$lis[0].type) === type;
};
...
const moveListItemUp = (editor, {
  ...
  const move = () => {
    ...
    const movedUpLiPath = slate.Path.next(liParentPath);
...
const moveListItems = (editor, {
  increase = true,
  at = (_editor$selection => (_editor$selection = editor.selection) !== null && _editor$selection !== void 0 ? _editor$selection : undefined)(),
...
const removeListItem = (editor, {
  ...
  plateCommon.withoutNormalizing(editor, () => {
    ...
    if (previousLiPath) {
      ...
      if (!tempLi) return;
      const tempLiPathRef = plateCommon.createPathRef(editor, tempLi[1]);
  ...
  if (res) {
    ...
    if (plateCommon.isSelectionAtBlockStart(editor, {
      ...
      plateCommon.withoutNormalizing(editor, () => {
        ...
        const pointBeforeListItem = plateCommon.getPointBefore(editor, editor.selection.focus);
        let currentLic;
        let hasMultipleChildren = false;

        // check if closest lic ancestor has multiple children
        if (pointBeforeListItem && isAcrossListItems({
          ...editor,
          selection: {
          ...
          const licType = plateCommon.getPluginType(editor, ELEMENT_LIC);
          const _licNodes = plateCommon.getNodeEntries(editor, {
            at: listItem[1],
            mode: 'lowest',
        ...
        const leftoverListItem = plateCommon.getNodeEntry(editor, slate.Path.parent(currentLic[1]));
...
const selectionIsInAListHandler = (editor, res, defaultDelete, unit) => {
  const {
    listItem
  } = res;
  ...
  if (!hasListChild(editor, listItem[0])) {
    ...
    const pointAfterListItem = plateCommon.getPointAfter(editor, editor.selection.focus);
    if (!pointAfterListItem || !isAcrossListItems({
      ...editor,
      selection: {
        anchor: editor.selection.anchor,
    ...
    defaultDelete(unit);
    const leftoverListItem = plateCommon.getNodeEntry(editor, slate.Path.parent(nextSelectableLic[1]));
    if (leftoverListItem && leftoverListItem[0].children.length === 0) {
      // remove the leftover empty list item
      plateCommon.removeNodes(editor, {
        at: leftoverListItem[1]
...
const deleteForwardList = (editor, defaultDelete, unit) => {
  ...
  plateCommon.withoutNormalizing(editor, () => {
    ...
    skipDefaultDelete = selectionIsInAListHandler(editor, res, defaultDelete, unit);
  });
  return skipDefaultDelete;
};
...
const getLiStart = editor => {
  ...
  return plateCommon.getAboveNode(editor, {
    at: start,
    match: {
      type: plateCommon.getPluginType(editor, ELEMENT_LI)
    }
...
const insertFragmentList = editor => {
  ...
  const trimList = listRoot => {
    ...
    const commonAncestorEntry = textEntries.reduce((commonAncestor, textEntry) => slate.Path.isAncestor(commonAncestor[1], textEntry[1]) ? commonAncestor : plateCommon.getCommonNode(listRoot, textEntry[1], commonAncestor[1]),
...
const normalizeListItem = (editor, {
  ...
  let changed = false;
  const allValidLiChildrenTypes = new Set([plateCommon.getPluginType(editor, ELEMENT_UL), plateCommon.getPluginType(editor, ELEMENT_OL), plateCommon.getPluginType(editor, ELEMENT_LIC), ...validLiChildrenTypes]);
  const [, liPath] = listItem;
  const liChildren = plateCommon.getChildren(listItem);

  // Get invalid (type) li children path refs to be moved
  const invalidLiChildrenPathRefs = liChildren.filter(([child]) => !allValidLiChildrenTypes.has(child.type)).map(([, childPath]) => plateCommon.createPathRef(editor, childPath));
  ...
  if (licChildren.length > 0) {
    ...
    const to = slate.Path.next(licChildren.at(-1)[1]);
...
const normalizeList = (editor, {
  ...
  return ([node, path]) => {
    ...
    if (node.type === plateCommon.getPluginType(editor, ELEMENT_LI) && normalizeListItem(editor, {
      listItem: [node, path],
      validLiChildrenTypes
    })) {
      return;
    ...
    if (node.type === licType && licType !== defaultType && ((_getParentNode = plateCommon.getParentNode(editor, path)) === null || _getParentNode === void 0 ? void 0 : _getParentNode[0].type) !== liType) {
      plateCommon.setElements(editor, {
        type: defaultType
      }, {
        at: path
...
const useListToolbarButtonState = ({
  nodeType = ELEMENT_UL
} = {}) => {
  const editor = plateCommon.usePlateEditorState();
  const pressed = !!(editor !== null && editor !== void 0 && editor.selection) && plateCommon.someNode(editor, {
    match: {
      type: plateCommon.getPluginType(editor, nodeType)
    }
  });
  return {
    pressed,
...
exports.useListToolbarButton = useListToolbarButton;
exports.useListToolbarButtonState = useListToolbarButtonState;
exports.useTodoListElement = useTodoListElement;
exports.useTodoListElementState = useTodoListElementState;
