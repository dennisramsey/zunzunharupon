import { getParentNode, getPluginType, getAboveNode, isCollapsed, getNode, withoutNormalizing, deleteText, isStartPoint, isBlockTextEmptyAfterSelection, insertElements, getMarks, select, splitNodes, createPluginFactory, onKeyDownToggleElement, match, isRangeAcrossBlocks, someNode, isElement, getNodeEntry, wrapNodes, moveNodes, findDescendant, getLastChildPath, moveChildren, getBlockAbove, setElements, ELEMENT_DEFAULT, unwrapNodes, getCommonNode, isLastChild, getNodeEntries, createPathRef, collapseSelection, isExpanded, getPreviousPath, deleteMerge, removeNodes, getPluginOptions, findNode, Hotkeys, unhangRange, isSelectionAtBlockStart, isFirstChild, mockPlugin, getPointBefore, isSelectionAtBlockEnd, getPointAfter, getEditorString, getChildren, getEndPoint, getStartPoint, isBlockAboveEmpty, getPlugin, getNodeString, getNodeTexts, getNodes, isBlock, insertEmptyElement, KEY_DESERIALIZE_HTML, usePlateEditorState, usePlateEditorRef, focusEditor, findNodePath, setNodes } from '@udecode/plate-common';
  ...
  if (res) {
    ...
    if (isSelectionAtBlockStart(editor, {
      ...
      withoutNormalizing(editor, () => {
        ...
        const pointBeforeListItem = getPointBefore(editor, editor.selection.focus);
        let currentLic;
        ...
        const leftoverListItem = getNodeEntry(editor, Path.parent(currentLic[1]));
...
const selectionIsInAListHandler = (editor, res, defaultDelete, unit) => {
  ...
  if (!hasListChild(editor, listItem[0])) {
    ...
    defaultDelete(unit);
    const leftoverListItem = getNodeEntry(editor, Path.parent(nextSelectableLic[1]));
    if (leftoverListItem && leftoverListItem[0].children.length === 0) {
      // remove the leftover empty list item
      removeNodes(editor, {
        at: leftoverListItem[1]
...
const insertFragmentList = editor => {
  ...
  const trimList = listRoot => {
    ...
    const commonAncestorEntry = textEntries.reduce((commonAncestor, textEntry) => Path.isAncestor(commonAncestor[1], textEntry[1]) ? commonAncestor : getCommonNode(listRoot, textEntry[1], commonAncestor[1]),
...
const normalizeListItem = (editor, {
  ...
  let changed = false;
  const allValidLiChildrenTypes = new Set([getPluginType(editor, ELEMENT_UL), getPluginType(editor, ELEMENT_OL), getPluginType(editor, ELEMENT_LIC), ...validLiChildrenTypes]);
  const [, liPath] = listItem;
  const liChildren = getChildren(listItem);

  // Get invalid (type) li children path refs to be moved
  const invalidLiChildrenPathRefs = liChildren.filter(([child]) => !allValidLiChildrenTypes.has(child.type)).map(([, childPath]) => createPathRef(editor, childPath));
...
const normalizeList = (editor, {
  ...
  return ([node, path]) => {
    ...
    if (node.type === getPluginType(editor, ELEMENT_LI) && normalizeListItem(editor, {
      listItem: [node, path],
      validLiChildrenTypes
    })) {
      return;
    ...
    if (node.type === licType && licType !== defaultType && ((_getParentNode = getParentNode(editor, path)) === null || _getParentNode === void 0 ? void 0 : _getParentNode[0].type) !== liType) {
      setElements(editor, {
        type: defaultType
      }, {
        at: path
...
const useListToolbarButtonState = ({
  ...
  const pressed = !!(editor !== null && editor !== void 0 && editor.selection) && someNode(editor, {
    match: {
      type: getPluginType(editor, nodeType)
    }
  });
  return {
    pressed,
