import { traverseHtmlElements, isHtmlComment, removeHtmlNodesBetweenComments, NO_BREAK_SPACE, SPACE, CARRIAGE_RETURN, LINE_FEED, getHtmlComments, hexToBase64, isOlSymbol, parseHtmlElement, replaceTagName, TAB, preCleanHtml, cleanHtmlEmptyElements, cleanHtmlTextNodes, cleanHtmlBrElements, cleanHtmlLinkElements, cleanHtmlFontElements, copyBlockMarksToSpanChild, postCleanHtml, createPluginFactory, KEY_DESERIALIZE_HTML } from '@udecode/plate-common';
...
const getVShapeSpid = (document, element) => {
  if (element.tagName === 'IMG') {
    ...
    if (element.parentElement && element.parentElement.parentElement && element.parentElement.parentElement.innerHTML.includes('msEquation')) {
      return null;
    }
...
const cleanDocxListElements = rootNode => {
  traverseHtmlElements(rootNode, element => {
    ...
    if (styleAttribute) {
      element.setAttribute('style', styleAttribute.replaceAll(/mso-list:\s*ignore/gi, 'mso-list:Ignore'));
    }
    return true;
  });
};
...
const getTextListStyleType = text => {
  ...
  if ((_text$match2 = text.match(/^[cdilmvx]+\./)) !== null && _text$match2 !== void 0 && _text$match2[0]) {
  ...
  if ((_text$match4 = text.match(/^[CDILMVX]+\./)) !== null && _text$match4 !== void 0 && _text$match4[0]) {
...
const KEYS = [ELEMENT_PARAGRAPH, ELEMENT_H1, ELEMENT_H2, ELEMENT_H3, ELEMENT_H4, ELEMENT_H5, ELEMENT_H6];
const overrideByKey = {};
KEYS.forEach(key => {
  overrideByKey[key] = {
    then: (editor, {
...
export { KEY_DESERIALIZE_DOCX, cleanDocx, cleanDocxBrComments, cleanDocxEmptyParagraphs, cleanDocxFootnotes, cleanDocxImageElements, cleanDocxListElements, cleanDocxListElementsToList, cleanDocxQuotes, cleanDocxSpacerun, cleanDocxSpans, cleanDocxTabCount, createDeserializeDocxPlugin, docxListToList, generateSpaces, generateTabs, getDocxIndent, getDocxListContentHtml, getDocxListIndent, getDocxListNode, getDocxSpacing, getDocxTextIndent, getRtfImageHex, getRtfImageMimeType, getRtfImageSpid, getRtfImagesByType, getRtfImagesMap, getTextListStyleType, getVShapeSpid, getVShapes, isDocxBookmark, isDocxContent, isDocxFootnote, isDocxList, isDocxOl };
