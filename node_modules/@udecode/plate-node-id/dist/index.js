...
const withNodeId = (editor, {
  ...
  const filterNode = nodeEntry => {
    var _nodeEntry$;
    return filter(nodeEntry) && (!filterText || ((_nodeEntry$ = nodeEntry[0]) === null || _nodeEntry$ === void 0 ? void 0 : _nodeEntry$.type) !== undefined);
  };
  const removeIdFromNodeIfDuplicate = node => {
    if (!reuseId || plateCommon.someNode(editor, {
      match: {
        [idKey]: node[idKey]
      },
      at: []
  ...
  const overrideIdIfSet = node => {
    if (plateCommon.isDefined(node._id)) {
      const id = node._id;
      delete node._id;
  ...
  editor.insertNodes = (_nodes, options) => {
    const nodes = castArray_1(_nodes);
  ...
  editor.apply = operation => {
    if (operation.type === 'insert_node') {
      ...
      plateCommon.defaultsDeepToNodes({
        node,
        path: operation.path,
    ...
    if (operation.type === 'split_node') {
      ...
      if (plateCommon.queryNode([node, operation.path], query)) {
        /**
         * Create a new id if:
         * - the id in the new node is already being used in the editor or,
         * - the node has no id
         */
        if (!reuseId || id === undefined || plateCommon.someNode(editor, {
          match: {
            [idKey]: id
          },
          at: []
 ...
 * @see {@link withNodeId}
 */
const createNodeIdPlugin = plateCommon.createPluginFactory({
  key: KEY_NODE_ID,
...
exports.KEY_NODE_ID = KEY_NODE_ID;
exports.createNodeIdPlugin = createNodeIdPlugin;
exports.withNodeId = withNodeId;
