...
const onKeyDownResetNode = (editor, {
  ...
  if (plateCommon.isCollapsed(editor.selection)) {
    rules.forEach(({
      ...
      if (hotkey && isHotkey(hotkey, event) && predicate(editor) && plateCommon.someNode(editor, {
        match: {
          type: types
        }
      })) {
 ...
 */
const createResetNodePlugin = plateCommon.createPluginFactory({
  key: KEY_RESET_NODE,
  handlers: {
  ...
  withOverrides: (editor, {
    ...
    if (!options.disableEditorReset) {
      const deleteFragmentPlugin = () => {
        ...
        if (!selection) return;
        const start = plateCommon.getStartPoint(editor, []);
        const end = plateCommon.getEndPoint(editor, []);
        if (slate.Point.equals(selection.anchor, start) && slate.Point.equals(selection.focus, end) || slate.Point.equals(selection.focus, start) && slate.Point.equals(selection.anchor, end)) {
          plateCommon.resetEditorChildren(editor, {
            insertOptions: {
              select: true
            }
          });
      ...
      editor.deleteBackward = unit => {
        ...
        if (selection && plateCommon.isCollapsed(selection)) {
          ...
          if (slate.Point.equals(selection.anchor, start)) {
            const node = plateCommon.getNode(editor, [0]);
            const {
              children,
              ...props
            ...
            plateCommon.withoutNormalizing(editor, () => {
              plateCommon.unsetNodes(editor, Object.keys(plateCommon.getNodeProps(node)), {
                at: [0]
              });
...
exports.KEY_RESET_NODE = KEY_RESET_NODE;
exports.SIMULATE_BACKSPACE = SIMULATE_BACKSPACE;
exports.createResetNodePlugin = createResetNodePlugin;
exports.onKeyDownResetNode = onKeyDownResetNode;
