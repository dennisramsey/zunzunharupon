...
const createElementWithSlate = (slateProps, dndWrapper) => {
  ...
  if (dndWrapper) {
    return /*#__PURE__*/React.createElement(dndWrapper, null, /*#__PURE__*/React.createElement(slateReact.Slate, {
      editor,
      initialValue: value,
      onChange,
      ...props
...
const elementToHtml = (editor, {
  ...
  editor.plugins.some(plugin => {
    ...
    if (!plugin.isElement || plugin.serializeHtml === null || props.element.type !== plugin.type) return false;

    // Render element using picked plugins renderElement function and ReactDOM
    html = htmlEntities.decode(server.renderToStaticMarkup(createElementWithSlate({
      ...slateProps,
...
const leafToHtml = (editor, {
  ...
  return editor.plugins.reduce((result, plugin) => {
    ...
    props = {
      ...plateCommon.pipeInjectProps(editor, props),
      children: result
    };
    ...
    let html = htmlEntities.decode(server.renderToStaticMarkup(createElementWithSlate({
      ...slateProps,
      children: serialized
    })));
    html = stripClassNames(html, {
...
const newLinesToHtmlBr = html => html.replaceAll('\n', '<br />');

// Remove redundant data attributes
const stripSlateDataAttributes = rawHtml => rawHtml.replaceAll(/( data-slate)(-node|-type|-leaf)="[^"]+"/g, '').replaceAll(/( data-testid)="[^"]+"/g, '');

// Remove extra whitespace generated by ReactDOMServer
const trimWhitespace = rawHtml => rawHtml.replaceAll(/(\r\n|[\t\n\r])/g, '');
