...
const findTabDestination = (editor, {
  ...
  const nextTabbableEntry = direction === 'forward' ? tabbableEntries.find(entry => !slate.Path.isBefore(entry.path, selectionPath)) : [...tabbableEntries].reverse().find(entry => slate.Path.isBefore(entry.path, selectionPath));
...
function TabbableEffects() {
  ...
  react.useEffect(() => {
    ...
    const handler = event => {
      var _ref;
      if (event.key !== 'Tab' || event.defaultPrevented || !(query !== null && query !== void 0 && query(editor, event))) {
        return;
      }
      const insertedTabbableEntries = insertTabbableEntries === null || insertTabbableEntries === void 0 ? void 0 : insertTabbableEntries(editor, event);
      ...
      }).filter(entry => entry && (isTabbable === null || isTabbable === void 0 ? void 0 : isTabbable(editor, entry)));
