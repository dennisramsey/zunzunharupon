...
var slateReact$1 = require('@udecode/slate-react');
...
const getStoreIndex = (name = '') => name.length > 0 ? `${name}Store` : 'store';
...
function ElementProvider({
  ...
  return /*#__PURE__*/React.createElement(jotai.Provider, _extends({
    initialValues: [[elementStore.atom.element, element]],
    scope: SCOPE_ELEMENT
  }, props), /*#__PURE__*/React.createElement(jotai.Provider, _extends({
    initialValues: [[elementStore.atom.element, element]],
    scope: scope
...
function DefaultLeaf({
  ...
  return /*#__PURE__*/React.createElement("span", _extends({}, attributes, props), children);
}
...
const plateIdAtom = jotai.atom(PLATE_SCOPE);

/**
 * Get the closest `Plate` id.
 */
const usePlateId = () => jotai.useAtom(plateIdAtom, GLOBAL_PLATE_SCOPE)[0];
const createPlateStore = ({
  ...
  return {
    ...
    usePlateStore: _id => {
      ...
      if (utils.isDefined(_id) || stores.usePlateStore(_id).get.id(_id)) {
...
const applyDeepToNodes = ({
  ...
  if (slate.queryNode(entry, query)) {
  ...
  if (!slate.isAncestor(node)) return;
...
const getBlockAbove = (editor, options = {}) => slate.getAboveNode(editor, {
  ...options,
  block: true
});
const isSelectionAtBlockStart = (editor, options) => {
  ...
  return slate.isStartPoint(editor, selection.focus, path) || slate.isExpanded(editor.selection) && slate.isStartPoint(editor, selection.anchor, path);
};
const removeSelectionMark = editor => {
  const marks = slate.getMarks(editor);
  if (!marks) return;

  // remove all marks
  Object.keys(marks).forEach(key => {
    slate.removeEditorMark(editor, key);
  });
};
...
const isInlineNode = editor => node => slate.isText(node) || slate.isElement(node) && editor.isInline(node);
...
var resolveDescendants = children => {
  ...
  var addChild = child => {
    ...
    if (slate$1.Text.isText(child)) {
      var c = child; // HACK: fix typescript complaining

      if (slate$1.Text.isText(prev) && STRINGS.has(prev) && STRINGS.has(c) && slate$1.Text.equals(prev, c, {
        loose: true
      })) {
        prev.text += c.text;
      } else {
    ...
    } else if (slate$1.Element.isElement(child)) {
      nodes.push(child);
    } else if (child instanceof Token) {
      var n = nodes[nodes.length - 1];

      if (!slate$1.Text.isText(n)) {
        addChild('');
...
var createEditor = makeEditor => (tagName, attributes, children) => {
  ...
  for (var child of children) {
    if (slate$1.Range.isRange(child)) {
  ...
  for (var [node, path] of slate$1.Node.texts(editor)) {
  ...
  } else if (slate$1.Range.isRange(selection)) {
...
var DEFAULT_CREATORS = {
  anchor: createAnchor,
  cursor: createCursor,
  editor: createEditor(slate$1.createEditor),
...
const deserializeHtmlNodeChildren = (editor, node) => Array.from(node.childNodes).flatMap(deserializeHtmlNode(editor));
      ...
      if (validAttribute) {
        if (typeof validAttribute === 'string') {
          ...
          for (const [attributeName, attributeValue] of Object.entries(validAttribute)) {
            ...
            if (!utils.isDefined(elAttribute) || !attributeValues.includes(elAttribute)) return false;
          }
        }
      }
      return true;
...
const htmlElementToLeaf = (editor, element) => {
  ...
  return deserializeHtmlNodeChildren(editor, element).reduce((arr, child) => {
    if (!child) return arr;
    if (slate.isElement(child)) {
      if (Object.keys(node).length > 0) {
        mergeDeepToNodes({
          node: child,
          source: node,
          query: {
            filter: ([n]) => slate.isText(n)
          }
        });
      }
      arr.push(child);
      ...
      if (slate.isText(child) && child.text) {
...
const create = key => {
  ...
  return event => {
    ...
    if (utils.IS_APPLE && isApple && isApple(event)) return true;
    if (!utils.IS_APPLE && isWindows && isWindows(event)) return true;
    return false;
  };
};
const createComposing = key => (editor, event, {
  ...
  if (!!composing !== slateReact$1.isComposing(editor)) return false;
  return true;
};
const Hotkeys = {
  isBold: create('bold'),
...
const pluginInjectProps = (editor, {
  ...
  const queryResult = query === null || query === void 0 ? void 0 : query(props, nodeProps);
  if (!queryResult && validTypes && slate.isElement(node) && node.type && !validTypes.includes(node.type)) {
    return;
  }
  const nodeValue = node[nodeKey];

  // early return if there is no reason to inject props
  if (!queryResult && (!utils.isDefined(nodeValue) || validNodeValues && !validNodeValues.includes(nodeValue) || nodeValue === defaultNodeValue)) {
    return;
  }
  ...
  if (transformProps) {
    var _transformProps;
    res = (_transformProps = transformProps(transformOptions, res)) !== null && _transformProps !== void 0 ? _transformProps : res;
  }
  return res;
};
...
const pipeRenderElement = (editor, renderElementProp) => {
  ...
  return function render(nodeProps) {
    ...
    return /*#__PURE__*/React.createElement(slateReact.DefaultElement, props);
  };
};
...
const withHOC = (HOC, Component, hocProps) => function hoc(props) {
  return /*#__PURE__*/React.createElement(HOC, hocProps, /*#__PURE__*/React.createElement(Component, props));
};
...
const useEditableProps = ({
  ...
  return useDeepCompare.useDeepCompareMemo(() => ({
...
const EditorMethodsEffect = ({
  ...
  const storeSetters = Object.fromEntries(EXPOSED_STORE_KEYS.map(key => [key, plateStore.set[key]()]));
...
function PlateEditable({
  ...
  const editable = /*#__PURE__*/React.createElement(slateReact.Editable, _extends({
    ref: editableRef
  }, editableProps));
...
function PlateProviderContent({
  ...
  const editor = React.useMemo(() => _editor !== null && _editor !== void 0 ? _editor : createPlateEditor({
    id,
    plugins: _plugins,
    disableCorePlugins
  }),
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  []);
  return /*#__PURE__*/React.createElement(jotai.Provider, {
...
function PlateSlate({
  ...
  let aboveSlate = /*#__PURE__*/React.createElement(slateReact.Slate, slateProps, children);
...
const toggleNodeType = (editor, options, editorNodesOptions) => {
  ...
  const isActive = slate.someNode(editor, {
    ...editorNodesOptions,
    match: {
      type: activeType
    }
  ...
  slate.setElements(editor, {
    type: isActive ? inactiveType : activeType
  });
};

Object.defineProperty(exports, 'JotaiProvider', {
  enumerable: true,
  get: function () { return jotai.Provider; }
});
...
Object.defineProperty(exports, 'nanoid', {
  enumerable: true,
  get: function () { return nanoid.nanoid; }
});
...
Object.defineProperty(exports, 'useHotkeys', {
  enumerable: true,
  get: function () { return reactHotkeysHook.useHotkeys; }
});
...
exports.EXPOSED_STORE_KEYS = EXPOSED_STORE_KEYS;
exports.EditorMethodsEffect = EditorMethodsEffect;
...
exports.GLOBAL_PLATE_SCOPE = GLOBAL_PLATE_SCOPE;
exports.Hotkeys = Hotkeys;
exports.KEY_DESERIALIZE_AST = KEY_DESERIALIZE_AST;
exports.KEY_DESERIALIZE_HTML = KEY_DESERIALIZE_HTML;
exports.KEY_EDITOR_PROTOCOL = KEY_EDITOR_PROTOCOL;
exports.KEY_EVENT_EDITOR = KEY_EVENT_EDITOR;
exports.KEY_INLINE_VOID = KEY_INLINE_VOID;
exports.KEY_INSERT_DATA = KEY_INSERT_DATA;
exports.KEY_NODE_FACTORY = KEY_NODE_FACTORY;
exports.KEY_PREV_SELECTION = KEY_PREV_SELECTION;
exports.LINE_FEED = LINE_FEED;
exports.NO_BREAK_SPACE = NO_BREAK_SPACE;
exports.PLATE_SCOPE = PLATE_SCOPE;
exports.Plate = Plate;
exports.PlateEditable = PlateEditable;
exports.PlateProvider = PlateProvider;
exports.PlateProviderEffects = PlateProviderEffects;
exports.PlateSlate = PlateSlate;
exports.PlateTest = PlateTest;
exports.SCOPE_ELEMENT = SCOPE_ELEMENT;
exports.SPACE = SPACE;
exports.TAB = TAB;
...
exports.createEditorProtocolPlugin = createEditorProtocolPlugin;
...
exports.createNodeFactoryPlugin = createNodeFactoryPlugin;
...
exports.createPrevSelectionPlugin = createPrevSelectionPlugin;
...
exports.getEventPlateId = getEventPlateId;
exports.getHtmlComments = getHtmlComments;
exports.getInjectedPlugins = getInjectedPlugins;
exports.getJotaiProviderInitialValues = getJotaiProviderInitialValues;
...
exports.isHtmlTable = isHtmlTable;
...
exports.usePlateActions = usePlateActions;
...
exports.usePlateReadOnly = usePlateReadOnly;
exports.usePlateSelection = usePlateSelection;
exports.usePlateSelectors = usePlateSelectors;
exports.usePlateStates = usePlateStates;
exports.usePlateStore = usePlateStore;
exports.useRedecorate = useRedecorate;
exports.useResetPlateEditor = useResetPlateEditor;
exports.useSlateProps = useSlateProps;
exports.useUpdatePlateKey = useUpdatePlateKey;
exports.withEditorProtocol = withEditorProtocol;
