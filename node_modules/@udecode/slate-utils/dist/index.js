...
var slate$1 = require('@udecode/slate');
...
const findDescendant = (editor, options) => {
  ...
  try {
    ...
    if (slate.Span.isSpan(at)) {
      ...
      const first = slate$1.getPath(editor, at, {
        edge: 'start'
      });
      const last = slate$1.getPath(editor, at, {
        edge: 'end'
      });
    ...
    if (slate.Path.isPath(at)) {
      root = slate$1.getNodeEntry(editor, at);
    }
    const nodeEntries = slate$1.getNodeDescendants(root[0], {
      reverse,
      from,
      to,
      pass: ([n]) => voids ? false : slate$1.isVoid(editor, n)
    });
    for (const [node, path] of nodeEntries) {
      if (slate$1.match(node, path, _match)) {
 ...
 */
const getBlockAbove = (editor, options = {}) => slate$1.getAboveNode(editor, {
  ...options,
  block: true
});
...
const getEdgeBlocksAbove = (editor, {
  ...
  if (!at) return null;
  const [start, end] = slate$1.getEdgePoints(editor, at !== null && at !== void 0 ? at : editor.selection);
...
const getLastChild$1 = nodeEntry => {
  const [node, path] = nodeEntry;
  if (slate$1.isText(node)) return null;
...
const getLastChild = (node, level) => {
  if (!(level + 1) || !slate$1.isAncestor(node)) return node;
  const {
    children
  } = node;
...
const getLastNodeByLevel = (editor, level) => {
  ...
  const lastNode = children.at(-1);
  if (!lastNode) return;
  const [, lastPath] = slate$1.getLastNode(editor, []);
 ...
 */
const getMark = (editor, key) => {
  if (!editor) return;
  const marks = slate$1.getMarks(editor);
...
const getNextNodeStartPoint = (editor, at) => {
  const nextEntry = slate$1.getNextNode(editor, {
    at
  });
  if (!nextEntry) return;
  return slate$1.getStartPoint(editor, nextEntry[1]);
};
...
const isRangeAcrossBlocks = (editor, {
  ...
  return !slate.Path.equals(startBlock[1], endBlock[1]);
};
...
const getPointBeforeLocation = (editor, at, options) => {
  if (!options || !options.match && !options.matchString) {
    return slate$1.getPointBefore(editor, at, options);
  }
  ...
  matchStrings.some(matchString => {
    let beforeAt = at;
    let previousBeforePoint = slate$1.getPoint(editor, at, {
      edge: 'end'
    });
    ...
    while (true) {
      var _options$match;
      const beforePoint = slate$1.getPointBefore(editor, beforeAt, options);
      ...
      const beforeString = slate$1.getEditorString(editor, {
        anchor: beforePoint,
        focus: previousBeforePoint
      });
      ...
      if (matchString === beforeStringToMatch || (_options$match = options.match) !== null && _options$match !== void 0 && _options$match.call(options, {
        ...
        if (options.afterMatch) {
          if (stackLength && unitOffset) {
            var _stack$at;
            point = (_stack$at = stack.at(-1)) === null || _stack$at === void 0 ? void 0 : _stack$at.point;
            return !!point;
          }
      ...
      count += 1;
      if (!options.skipInvalid && (!matchString || count >= matchString.length)) return;
    }
  });
  return point;
};
...
const getPointFromLocation = (editor, {
  ...
  let point;
  if (slate.Range.isRange(at)) point = focus ? at.focus : at.anchor;
...
const getPointNextToVoid = (editor, {
  ...
  const startVoid = slate$1.getVoidNode(editor, {
    at,
    mode: 'highest'
  });
  if (startVoid) {
    ...
    if (blockAbove) {
      const nextPoint = after ? slate$1.getPointAfter(editor, at) : slate$1.getPointBefore(editor, at);
...
const getPreviousBlockById = (editor, id, query) => {
  const entry = slate$1.findNode(editor, {
    match: {
      id
    }
  });
  if (entry) {
    const prevEntry = slate$1.getPreviousNode(editor, {
      at: entry[1]
    });
    if (prevEntry && prevEntry[0].id && slate$1.isBlock(editor, prevEntry[0])) {
      return prevEntry;
    }
  }
  let found = false;
  const _nodes = slate$1.getNodeEntries(editor, {
    mode: 'highest',
    reverse: true,
    match: n => {
      // filter nodes that are not blocks and without id.
      if (!slate$1.isBlock(editor, n) || !n.id) return false;
      ...
      return found && n.id !== id && slate$1.queryNode([n, []], query);
    },
    at: []
  });
  ...
  if (!found) return;
  const _entries = slate$1.getNodeEntries(editor, {
    mode: 'highest',
    match: n => {
      return slate$1.isBlock(editor, n) && !!n.id && slate$1.queryNode([n, []], query);
    },
    at: []
  });
...
const getPreviousNodeEndPoint = (editor, at) => {
  ...
  if (!prevEntry) return;
  return slate$1.getEndPoint(editor, prevEntry[1]);
};
...
const getPreviousSiblingNode = (editor, path) => {
  ...
  if (index > 0) {
    ...
    const previousSiblingNode = slate$1.getNode(editor, previousSiblingPath);
    return previousSiblingNode ? [previousSiblingNode, previousSiblingPath] : undefined;
  }
};
...
const getRangeBefore = (editor, at, options) => {
  ...
  if (!anchor) return;
  const focus = slate$1.getPoint(editor, at, {
    edge: 'end'
  });
  return {
    anchor,
...
const getRangeFromBlockStart = (editor, options = {}) => {
  ...
  if (!path) return;
  const start = slate$1.getStartPoint(editor, path);
 ...
 */
const getSelectionText = editor => slate$1.getEditorString(editor, editor.selection);

/**
 * Is an ancestor empty (empty text and no inline children).
 */
const isAncestorEmpty = (editor, node) => !slate$1.getNodeString(node) && !node.children.some(n => slate$1.isInline(editor, n));
...
const isBlockTextEmptyAfterSelection = editor => {
  ...
  const selectionParentEntry = slate$1.getParentNode(editor, editor.selection);
  if (!selectionParentEntry) return false;
  const [, selectionParentPath] = selectionParentEntry;
  if (!slate$1.isEndPoint(editor, cursor, selectionParentPath)) return false;
  const siblingNodes = getNextSiblingNodes(blockAbove, cursor.path);
  if (siblingNodes.length > 0) {
    for (const siblingNode of siblingNodes) {
      if (slate$1.isText(siblingNode) && siblingNode.text) {
        return false;
      }
    }
  } else {
    return slate$1.isEndPoint(editor, cursor, blockAbove[1]);
  }
  return true;
};

const isDocumentEnd = editor => {
  if (editor.selection) {
    const point = editor.selection.focus;
    const endPoint = slate$1.getEndPoint(editor, []);
    return endPoint.offset === 0 && slate$1.isEndPoint(editor, point, point) && slate.Path.equals(slate.Path.next(slate.Path.parent(point.path)), endPoint.path);
  }
  return false;
};
 ...
 */
const isFirstChild = path => path.at(-1) === 0;

/**
 * Is the mark defined in the selection.
 */
const isMarkActive = (editor, type) => {
  return utils.isDefined(getMark(editor, type));
};
...
const isPointAtWordEnd = (editor, {
  ...
  // Point after at
  const after = slate$1.getPointAfter(editor, at);

  // From at to after
  const afterRange = slate$1.getRange(editor, at, after);
  const afterText = slate$1.getEditorString(editor, afterRange);
...
const isSelectionAtBlockEnd = (editor, options) => {
  ...
  const path = (_getBlockAbove = getBlockAbove(editor, options)) === null || _getBlockAbove === void 0 ? void 0 : _getBlockAbove[1];
  return !!path && slate$1.isEndPoint(editor, (_editor$selection = editor.selection) === null || _editor$selection === void 0 ? void 0 : _editor$selection.focus, path);
};
...
const isSelectionAtBlockStart = (editor, options) => {
  ...
  return slate$1.isStartPoint(editor, selection.focus, path) || slate$1.isExpanded(editor.selection) && slate$1.isStartPoint(editor, selection.anchor, path);
};

/**
 * Is the selection expanded.
 */
const isSelectionExpanded = editor => slate$1.isExpanded(editor.selection);

const isTextByPath = (editor, path) => {
  const node = slate$1.getNode(editor, path);
  return slate$1.isText(node);
};
...
const isWordAfterTrigger = (editor, {
  ...
  const wordBefore = slate$1.getPointBefore(editor, at, {
    unit: 'word'
  });

  // Point before wordBefore
  const before = wordBefore && slate$1.getPointBefore(editor, wordBefore);

  // Range from before to start
  const beforeRange = before && slate$1.getRange(editor, before, at);

  // Before text
  const beforeText = slate$1.getEditorString(editor, beforeRange);

  // Starts with char and ends with word characters
  const escapedTrigger = utils.escapeRegExp(trigger);
  const beforeRegex = new RegExp(`^${escapedTrigger}([\\w|À-ÖØ-öø-ÿ|а-яА-ЯёЁ]+)$`);
...
const queryEditor = (editor, {
  ...
  if (allows.length > 0 && !slate$1.someNode(editor, {
    at,
    match: {
      type: allows
    }
  ...
  if (excludes.length > 0 && slate$1.someNode(editor, {
    at,
    match: {
      type: excludes
    }
...
const insertElements = (editor, nodes, options) => {
  slate$1.insertNodes(editor, nodes, options);
};
...
const insertEmptyElement = (editor, type, options) => {
  ...
  }, slate$1.getQueryOptions(editor, options));
};
...
const moveChildren = (editor, {
  ...
  const parentNode = slate.Path.isPath(at) ? slate$1.getNode(editor, parentPath) : at[0];
  if (!parentNode) return moved;
  if (!slate$1.isBlock(editor, parentNode)) return moved;
  for (let i = parentNode.children.length - 1; i >= fromStartIndex; i--) {
    const childPath = [...parentPath, i];
    const childNode = slate$1.getNode(editor, childPath);
    if (!match || childNode && match([childNode, childPath])) {
      slate$1.moveNodes(editor, {
        at: childPath,
        to
      });
      moved++;
...
const removeMark = (editor, {
  ...
  if (selection) {
    if (slate.Range.isRange(selection) && slate.Range.isExpanded(selection)) {
      slate$1.unsetNodes(editor, key, {
        at: selection,
        match: slate$1.isText,
        split: true,
        ...rest
      });
    } else if (editor.selection) {
      var _getMarks;
      const marks = (_getMarks = slate$1.getMarks(editor)) !== null && _getMarks !== void 0 ? _getMarks : {};
      key.forEach(k => {
        delete marks[k];
      });
      editor.marks = marks;
...
const removeNodeChildren = (editor, path, options) => {
  slate$1.withoutNormalizing(editor, () => {
    for (const [, childPath] of slate$1.getNodeChildren(editor, path, {
      reverse: true
    })) {
      slate$1.removeNodes(editor, {
        ...options,
        at: childPath
      });
    }
...
const removeSelectionMark = editor => {
  ...
  Object.keys(marks).forEach(key => {
    slate$1.removeEditorMark(editor, key);
  });
};
...
const replaceNodeChildren = (editor, {
  ...
  slate$1.withoutNormalizing(editor, () => {
    ...
    slate$1.insertNodes(editor, nodes, {
      ...insertOptions,
      at: at.concat([0])
    });
  });
...
const selectEndOfBlockAboveSelection = editor => {
  ...
  path && slate$1.select(editor, slate$1.getEndPoint(editor, path));
};
...
const toggleWrapNodes = (editor, type) => {
  if (slate$1.someNode(editor, {
    match: {
      type
    }
  })) {
    slate$1.unwrapNodes(editor, {
      match: {
        type
      }
    });
  } else {
    slate$1.wrapNodes(editor, {
      type,
      children: []
    });
  }
...
const wrapNodeChildren = (editor, element, options) => {
  ...
  slate$1.withoutNormalizing(editor, () => {
    ...
    slate$1.wrapNodes(editor, element, {
      ...options,
      at: firstChildPath
    });
...
exports.getNextNodeStartPoint = getNextNodeStartPoint;
...
exports.getPreviousNodeEndPoint = getPreviousNodeEndPoint;
exports.getPreviousPath = getPreviousPath;
exports.getPreviousSiblingNode = getPreviousSiblingNode;
...
exports.removeSelectionMark = removeSelectionMark;
...
exports.wrapNodeChildren = wrapNodeChildren;
